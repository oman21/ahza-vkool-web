...
ChatVisitorClass.prototype.UpdateVisitorInformation = function(_visitorId) {
    try
    {
        ...
        if(fdVisitor == null)
        {
            console.log("NO FULL USER DATA - LOAD");
        ...
        $(['d','e']).each(function()
        {
            ...
            if(render)
            {
                ...
                $('#visitor-info-offline-indicator-' + $(this)[0].toString()).css('display',visitor.is_active ? 'none': 'block');
                ...
                $('#visitor-info-'+elementId+'-placeholder-content-3').html('<div id="visitor-comment-'+elementId+'-list" data-role="none">' + that.CreateVisitorCommentTable(visitor, elementId) + '</div>').trigger('create');
                $('#visitor-info-'+elementId+'-placeholder-content-4').html('<div id="visitor-invitation-'+elementId+'-list" data-role="none">' + that.CreateVisitorInvitationTable(visitor, elementId) + '</div>').trigger('create');
                ...
                if(numberOfChats > 0 && !$('#visitor-info-'+elementId+'-placeholder-content-5').html().length)
                {
                    ...
                    var chatsHtml = lzm_chatDisplay.archiveDisplay.CreateMatchingChats(fdVisitor,fdVisitor.ArchivedChats,elementId) + '<fieldset class="lzm-fieldset" style="margin:0;padding:0;" id="chat-content-'+elementId+'-inner"></fieldset>';
                ...
                if(numberOfTickets > 0 && !$('#visitor-info-'+elementId+'-placeholder-content-6').html().length)
                {
                    ...
                    var ticketsHtml = lzm_chatDisplay.ticketDisplay.CreateMatchingTickets(fdVisitor,elementId) + '<fieldset class="lzm-fieldset" style="margin:0;" id="ticket-content-'+elementId+'-inner"></fieldset>';
                ...
                if(numberOfFeedbacks > 0 && !$('#visitor-info-'+elementId+'-placeholder-content-7').html().length)
                {
                    ...
                    var feedbacksHtml = lzm_chatDisplay.FeedbacksViewer.CreateMatchingFeedbacks(fdVisitor.ArchivedFeedbacks,elementId) + '<fieldset class="lzm-fieldset" style="margin:0;" id="feedback-content-'+elementId+'-inner"></fieldset>';
...
ChatVisitorClass.ShowVisitorInformation = function (_visitorId, _activeTab, _selectedChatId, _selectedTicketId, _dialog) {
    ...
    bodyString += '<div id="visitor-info-offline-indicator-'+((_dialog) ? 'd' : 'e')+'" class="visitor-info-offline-indicator">'+tid('offline')+'</div>';
    ...
    var tabsArray = [
        ...
        {name: t('Comments (<!--number_of_comments-->)', [['<!--number_of_comments-->', 0]]), content: ''/*commentsHtml*/},
        {name: t('Chat Invites (<!--number_of_invites-->)', [['<!--number_of_invites-->', 0]]), content: ''/*invitationsHtml*/},
