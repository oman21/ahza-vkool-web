...
ContextMenuClass.BuildMenu = function(event, _menu) {
    ...
    if ($('#' + menuId).length) {
        ...
        var wrapperDiv = $('<div></div>', {
            ...
            style: 'z-index:'+baseZIndex.toString()+';top:' + position.top + 'px;left: ' + position.left + 'px; min-width:160px; max-height: ' + Math.floor(lzm_chatDisplay.windowHeight* 0.6) + 'px; overflow:auto'
        });
        if (_menu.isSubmenu) {
